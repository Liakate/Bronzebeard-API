# Bronzebeard 3.3.5a API Scan — Readable Notes (from `BBApiScan 2.lua`)
...
- **Snapshot 1:** `2026-01-15 20:06:03`
  - Contains both:
    - `export_lines` (human-oriented text dump) including per-namespace function/non-function counts
    - `c_namespaces` (structured map of `C_*` namespaces → functions + non-function keys)
...
- Across those namespaces, the scan observed **1177 function keys** and **69 non-function keys**.
...
## Differences vs the earlier scan (`1.md`)

- **Throttling changed:** tickInterval `0.5s` (was `0.1s`), maxStepsPerTick `60` (was `120`).
- **Count change:** `C_CharacterAdvancement` functions `126 → 127` (nonfunc keys unchanged at `0`).
